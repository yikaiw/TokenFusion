...
class SetCriterion2d(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        """
        assert 'pred_logits2d' in outputs
        src_logits = outputs['pred_logits2d']
    ...
    def loss_cardinality(self, outputs, targets, indices, num_boxes):
        ...
        """
        pred_logits = outputs['pred_logits2d']
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        """
        assert 'pred_boxes2d' in outputs
        idx = self._get_src_permutation_idx(indices)
        src_boxes = outputs['pred_boxes2d'][idx]
    ...
    def get_iou(self, outputs, targets, indices):
        assert 'pred_boxes2d' in outputs
        idx = self._get_src_permutation_idx(indices)
        src_boxes = outputs['pred_boxes2d'][idx]
        target_boxes = torch.cat([t['boxes'][i] for t, (_, i) in zip(targets, indices)], dim=0)
    ...
    def forward(self, outputs, targets):
        ...
        for loss in losses.keys():
            outputs['loss2d_' + loss] = losses[loss]
        iou = torch.diag(self.get_iou(outputs, targets, indices))
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        """
        out_logits, out_bbox = outputs['pred_logits2d'], outputs['pred_boxes2d']
