...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape  # H, W == 530, 730
        # FIXME look at relaxing size constraints
        # assert H == self.img_size[0] and W == self.img_size[1], \
        #     f"Input image size ({H}*{W}) doesn't match model ({self.img_size[0]}*{self.img_size[1]})."
        x = self.proj(x)  # [8, 192, 33, 45]
        
        #print(self.proj.weight)
        x = x.flatten(2).transpose(1, 2)  # [8, 1485, 192]
        return x
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, is_distill=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # cls_tokens: [8, 1, embed_dim], x: [8, 1485, embed_dim], det_token: [8, 100, det_token]
        x = torch.cat((cls_tokens, x, det_token), dim=1)
        x = x + temp_pos_embed
        x = self.pos_drop(x)

        return x, temp_mid_pos_embed
        '''
...
def tiny(pretrained=None, **kwargs):
    model = VisionTransformer(
                ...
                drop_path_rate=0.3, norm_layer=partial(nn.LayerNorm, eps=1e-6))
    if pretrained: 
