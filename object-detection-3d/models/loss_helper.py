...
from .losses import smoothl1_loss, l1_loss, SigmoidFocalClassificationLoss, SetCriterion2d
from .matcher import build_matcher
from sunrgbd.sunrgbd_detection_dataset import viz_obb2d
...
def get_loss(end_points, epoch, config, num_decoder_layers,
    ...
    loss = query_points_generator_loss_coef * query_points_generation_loss + \
           1.0 / (num_decoder_layers + 1) * (obj_loss_coef * objectness_loss_sum + \
               box_loss_coef * box_loss_sum + sem_cls_loss_coef * sem_cls_loss_sum)

    weight_dict = {'loss_ce': 1, 'loss_bbox': 5, 'loss_giou': 2}
    matcher = build_matcher(weight_dict)
    criterion2d = SetCriterion2d(config.num_class, matcher=matcher, weight_dict=weight_dict, eos_coef=0.1, 
                                 losses=['labels', 'boxes'])
    # targets = [{'labels': end_points['sem_cls_label'][i], 'boxes': end_points['target_box2d'][i]} \
    #     for i in range(end_points['sem_cls_label'].shape[0])]

    # targets: list of dicts following DETR such that len(targets) == batch_size.
    targets = []
    for i in range(end_points['sem_cls_label'].shape[0]):
        zero_num = (end_points['target_box2d'][i].sum(dim=1) == 0).sum()
        targets.append({'labels': end_points['sem_cls_label'][i][:-zero_num], 
                        'boxes': end_points['target_box2d'][i][:-zero_num]})
    loss2d_dict, iou2d = criterion2d(end_points, targets)
    # iou2d.shape == sum_i(max_box_num - zero_num[i])
    losses2d = sum(loss2d_dict[k] * weight_dict[k] for k in loss2d_dict.keys() if k in weight_dict)

    '''
    img = end_points['full_img'][0].cpu().detach().numpy()
    target_box = targets[0]['boxes'].cpu().detach().numpy()
    #target_box[0::2] *= 730
    #target_box[1::2] *= 530
    #print(img)#img = img.transpose(1,2)
    if len(end_points['target_box2d'][0][:-zero_num]) > 0 and len(os.listdir('img')) < 50:
        #torch.save(end_points['full_img'][0].cpu().detach(), 'img.pth')
        viz_obb2d(img, target_box,  targets[0]['labels'].cpu().detach().numpy(), 'img')
    '''
    img = end_points['full_img'][0].cpu().detach().numpy()
    target_box = targets[0]['boxes'].cpu().detach().numpy()
    pred = end_points['pred_boxes2d'][0].cpu().detach().numpy()
    #print('pred len 2', len(pred))
    #target_box[0::2] *= 730
    #target_box[1::2] *= 530
    #print(img)#img = img.transpose(1,2)
