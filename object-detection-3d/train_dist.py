...
def parse_option():
    ...
    # Model
    parser.add_argument('--use_img', action='store_true', help='Use RGB.')
    ...
    parser.add_argument('--yolos_weight_decay', type=float, default=1e-4,
                        help='Optimization L2 weight decay for img_backbone [default: 1e-4]')
    parser.add_argument('--lr', type=float, default=0.004,
                        help='Initial learning rate for all except decoder and img_backbone [default: 0.004]')
    parser.add_argument('--decoder_lr', type=float, default=0.0002,
                        help='Initial learning rate for decoder [default: 0.0004]')
    parser.add_argument('--yolos_lr', type=float, default=5e-5,
                        help='Initial learning rate for img_backbone [default: 5e-5]')
...
def get_loader(args):
    ...
    if args.dataset == 'sunrgbd':
        ...
        TRAIN_DATASET = SunrgbdDetectionVotesDataset('train', num_points=args.num_point,
                                                     ...
                                                     scan_idx_list=os.path.join(BASE_DIR, 'sunrgbd_trainval/train_data_idx.txt'),
        ...
        TEST_DATASET = SunrgbdDetectionVotesDataset('val', num_points=args.num_point,
                                                    ...
                                                    use_height=True if args.use_height else False,
                                                    use_v1=(not args.use_sunrgbd_v2),
                                                    scan_idx_list=os.path.join(BASE_DIR, 'sunrgbd_trainval/val_data_idx.txt'),
                                                    data_root=args.data_root)
    ...
    if args.optimizer == 'adamW':
        param_dicts = [
            {"params": [p for n, p in model.named_parameters() \
                if "decoder" not in n and "img" not in n and p.requires_grad]},
            {"params": [p for n, p in model.named_parameters() if "decoder" in n and p.requires_grad],
             "lr": args.decoder_lr},
            {"params": [p for n, p in model.named_parameters() if "img" in n and p.requires_grad],
             "lr": args.yolos_lr, "weight_decay": args.yolos_weight_decay}
        ]
...
def train_one_epoch(epoch, train_loader, DATASET_CONFIG, model, criterion, optimizer, scheduler, args):
    ...
    for batch_idx, batch_data_label in enumerate(train_loader):
        ...
        if args.use_img:
            inputs.update({'scale': batch_data_label['scale'],
                           ...
                           'full_img': batch_data_label['full_img'],
                           'full_img_width': batch_data_label['full_img_width'],
                           'target_box2d': batch_data_label['target_box2d'],
                           })
        ...
        loss, end_points = criterion(end_points, epoch, DATASET_CONFIG,
                                     num_decoder_layers=args.num_decoder_layers,
        ...
        slim_loss = 0
        for mask in end_points['key_masks']:
        ...
        if (batch_idx + 1) % args.print_freq == 0:
            ...
            logger.info(''.join([f'{key} {stat_dict[key] / args.print_freq:.4f} \t'
            ...
            logger.info('Mean IoU2d %.2f' % end_points['iou'].data)
            logger.info(''.join([f'{key} {stat_dict[key] / args.print_freq:.4f} \t'
                                 for key in sorted(stat_dict.keys()) if 'loss2d_' in key]))

            for key in sorted(stat_dict.keys()):
                stat_dict[key] = 0


def evaluate_one_epoch(test_loader, DATASET_CONFIG, CONFIG_DICT, AP_IOU_THRESHOLDS, model, criterion, args):
    stat_dict = {}

    if args.num_decoder_layers > 0:
        prefixes = ['last_', 'proposal_'] + [f'{i}head_' for i in range(args.num_decoder_layers - 1)]
    else:
    ...
    for batch_idx, batch_data_label in enumerate(test_loader):
        ...
        for key in batch_data_label:
            assert (key not in end_points)
            end_points[key] = batch_data_label[key]
        loss, end_points = criterion(end_points, 0, DATASET_CONFIG,
                                     num_decoder_layers=args.num_decoder_layers,
                                     query_points_generator_loss_coef=args.query_points_generator_loss_coef,
                                     obj_loss_coef=args.obj_loss_coef,
                                     box_loss_coef=args.box_loss_coef,
        ...
        if (batch_idx + 1) % args.print_freq == 0:
            ...
            logger.info(''.join([f'{key} {stat_dict[key] / (float(batch_idx + 1)):.4f} \t'
                                 for key in sorted(stat_dict.keys()) if 'proposal_' in key]))
            logger.info('Mean IoU 2d: %.2f' % end_points['iou'].data)
            for ihead in range(args.num_decoder_layers - 2, -1, -1):
                logger.info(''.join([f'{key} {stat_dict[key] / (float(batch_idx + 1)):.4f} \t'
                                     for key in sorted(stat_dict.keys()) if f'{ihead}head_' in key]))
...
if __name__ == '__main__':
    ...
    while os.path.exists(LOG_DIR):
        LOG_DIR = os.path.join(opt.log_dir, 'group_free',
                               f'{opt.dataset}_{int(time.time())}', f'{np.random.randint(100000000)}')
    opt.log_dir = LOG_DIR
    os.makedirs(os.path.join(opt.log_dir, 'sunrgbd'), exist_ok=True)
    os.system('cp -r *.py utils models %s' % opt.log_dir)
    os.system('cp sunrgbd/*.py %s' % os.path.join(opt.log_dir, 'sunrgbd'))
