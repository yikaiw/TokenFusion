...
def write_ply_color(points, labels, filename, num_classes=None, colormap=pyplot.cm.jet):
    ...
    colors = ['#FD2B2B','#F9A11D','#CDFA19','#47FE19', '#17FD73','#30FCFC','#0C69F3','#3C0DF6','#CA1DF5','#F435A8']
    for i in range(N):
        idx = min(num_classes-1, labels[i])
        #print(idx, labels[i])
        c = Hex_to_RGB(colors[idx][1:])
        #c = colors[idx]
        #c = [int(x * 255) for x in c]
        vertex.append((points[i, 0], points[i, 1], points[i, 2], int(255*points[i, 3]), int(255*points[i, 4]), int(255*points[i, 5])))
...
def write_ply_rgb(points, out_filename, num_classes=None):
    """ Color (N,3) points with RGB colors (N,3) within range [0,255] as OBJ file """
    colors = [(points[i, 3], points[i, 4], points[i, 5]) for i in range(points.shape[0])]
    points = [(points[i, 0], points[i, 1], points[i, 2]) for i in range(points.shape[0])]
    # colors = colors.astype(int)
    N = len(points) #points.shape[0]
